<?php
include("config.php");
session_start(); // check bằng session để xem thằng user nó có đăng nhập vàp chưa, session có thể viết ở file common or config đều được
function check_login()
{
    //nếu chưa đăng nhập thì trở về trang chủ
    if (!isset($_SESSION["username"]) || empty($_SESSION["username"])) {
        js_alert("Đăng nhập để sử dụng chức năng này!");
        js_redirect_to("/");
    }
}
function is_logged()
{
    //return true nếu đã đăng nhập (có thông tin trong session username), đã đăng nhập thì không cho vào trang đăng nhập nữa 
...
function gen_option_ele($table, $col_value = "`id`", $col_text = "`name`", $selected_value = "")
{
    ...
    // option style= 'color: black', làm cho thẻ option có màu chữ là màu đen
    echo $str;
}
